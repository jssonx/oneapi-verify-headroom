...
static auto exception_handler = [](sycl::exception_list e_list) {
  for (std::exception_ptr const &e : e_list) {
    try {
      ...
      std::cout << "Exception in exception handler; Failure" << std::endl;
      std::terminate();
    }
  }
};
...
#if 1
 fprintf(stderr, "    [%d] Completed iteration %d, thread %d on GPU %d in %13.9f s. at timestamp %13.9f s.\n",
...
#if 1
 fprintf(stderr, "    [%d] Started initgpu() at timestamp %13.9f s.\n",
   thispid, tempus);
#endif
  try {
    ...
    std::cout << "Failure: exception trying to determine sycl device(s).\n";
    std::terminate();
  }
...
#if 1
 fprintf(stderr, "    [%d] Leaving initgpu() at timestamp %13.9f s.\n",
   thispid, tempus);
#endif
}
